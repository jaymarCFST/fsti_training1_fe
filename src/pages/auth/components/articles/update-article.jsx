...
const UpdateArticle = (props) => {
    ...
    const [selectedCategory, setSelectedCategory] = useState(props.data.article_category);
    ...
    async function updateArticle()
    {
        ...
        articleServices.updateArticle(data).catch(
            async(error) => {
                console.log(error)
            }
        ).then(
  ...
  return (
    <div className="App space-y-2">
        ...
        <div className="flex justify-start">
            <button onClick={updateArticle} className="px-3 py-2 text-white bg-indigo-500 rounded-md">Save Article</button>
        </div>
    </div>
  )
}
